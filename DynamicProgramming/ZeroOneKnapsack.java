...
public class ZeroOneKnapsack {
	...
	public static void maxValue(int[] arr1 , int[] arr2 , int target) {
		...
		int[][] dp = new int[arr1.length+1][target+1];
		...
		for (int i = 1 ; i <= m ; i++) {
			...
			for (int j = 1; j <=n ; j++) {
				...
				if (j<y) { dp[i][j] = dp[i-1][j];}
				
				else {
				    	int max1 = dp[i-1][j];                //If it opts not to be put
					    int max2 = x + dp[i-1][j-y];          //If it opts to be put into bag
